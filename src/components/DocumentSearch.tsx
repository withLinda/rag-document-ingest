...
export function DocumentSearch() {
  ...
  const [filterMetadata, setFilterMetadata] = useState<Record<string, string>>(
    {}
  );
  ...
  const handleSearch = useCallback(
    async (searchQuery: string) => {
      ...
      try {
        const response = await searchDocuments({
          ...
          filter:
            Object.keys(filterMetadata).length > 0
              ? {
    ...
    [hybridConfig, searchConfig, filterMetadata]
  );
  ...
  const renderSearchConfig = () => (
    <div className="mb-4 p-4 bg-gray-50 rounded-lg">
      ...
      <div className="flex gap-4">
          ...
          <input
            ...
            onChange={(e) =>
              setSearchConfig((prev) => ({
                ...prev,
                topK: Math.max(1, Math.min(20, parseInt(e.target.value) || 1)),
              }))
            }
        ...
        <div className="flex-1">
          <label className="block text-sm text-gray-600">Score Threshold</label>
          <input
            type="range"
            min="0"
            max="1"
      ...
      <div className="mt-4">
        ...
        >
          <span>{showMetadataFilters ? "▼" : "▶"} Metadata Filters</span>
        </button>

        {showMetadataFilters && (
          <div className="mt-4">
            <MetadataInput onChange={setFilterMetadata} initialMetadata={{}} />
          </div>
        )}
      </div>
    </div>
    ...
    <div className="bg-white rounded-xl shadow-sm p-6 mt-6">
      ...
      {debugInfo && (
        <div className="mt-6 p-4 bg-gray-50 rounded-lg text-sm text-gray-600">
          ...
          <div className="mb-1">
            Namespace ID: {import.meta.env.VITE_NAMESPACE_ID}
          </div>
